...
# header
# TRUE / FALSE
# Czy pierwszy wiersz to nagłówki?
#   sep
# ";" "," "\t" " "
# Separator kolumn (średnik, przecinek, tab, spacja)
# dec
# "." ","
# Separator dziesiętny (kropka lub przecinek)
# Import pliku CSV
# Funkcja read.table() domyślnie zakłada:
#   header = FALSE
# sep = "" (spacje)
# dec = "."
...
# [poniższe sekcje możesz skopiować i wkleić do nowego skryptu R, żeby wykonywać kod]
# 1.Podgląd danych
...
library(ggplot2)
# 13.Prosty wykres punktowy: urbanizacja a PKB per capita 
ggplot(kraje, aes(x = Urbanizacja_proc., y = PKB_per_capita)) +
  ...
  labs(
    ...
    x = "Urbanizacja (%)",
    y = "PKB per capita")
# 14.Zaawansowany wykres punktowy: urbanizacja a PKB per capita 
...
# 15.Zaawansowany wykres punktowy: rozmiar gospodarki a populacja
ggplot(kraje, aes(x = Populacja_mln, y = PKB, size = PKB_per_capita, color = Region)) +
  ...
  theme_minimal()
# 16.Prosty wykres słupkowy: liczba krajów w regionach
...
# 17.Zaawansowany wykres słupkowy poziomy: TOP 15 najbogatszych krajów
kraje %>%
  arrange(desc(PKB_per_capita)) %>%
  head(15) %>%
...
# 18.Wykres pudełkowy (boxplot): dostęp do internetu według regionów
...
# 19.Wykres pudełkowy (boxplot): przyrost populacji według regionów
